...
### HMR segments
hmrSegments <- readRDS("../data/segmentsHMR_extended.rds")
...
for(ff in 1:length(chr3Files)){ # loop timepoints
  ...
  avMethMatHMRNormalized <- sweep(avMethMatHMR, 2, STATS=apply(avMethMatHMR, 2, max), FUN="/")
  assign(paste0("avMethMatHMRNormalized", ff), avMethMatHMRNormalized)
}

# for some reason, there are 4 bins that do not have any data in the 4th timepoint:
# TODO: check this
rowSums(is.na(avMethMatHMRNormalized4))
...
for(ff in 1:length(chr3Files)){
  ...
  for(bb in 1:nrow(hmrSegments)){
    jointProbability <- colMeans(curMeth[,bb] * curMeth) # P(bin A, bin B)
    # P(bin A | bin B) = P(bin A, bin B) / P(bin B)
    conditionalProbability[,bb] <- jointProbability / colMeans(curMeth) 
...
plot(x=colMeans(avMethMatHMRNormalized1), y=diag(conditionalProbabilityTable1))
...
# make probability matrix: relative to max avg methylation in each bin.
### HML segments
HMLSegments <- readRDS("../data/segmentsHML_extended.rds")
...
for(ff in 1:length(chr3Files)){ # loop timepoints
  ...
  # normalize by maximum average methylation in each bin
  avMethMatHMLNormalized <- sweep(avMethMatHML, 2, STATS=apply(avMethMatHML, 2, max), FUN="/")
  assign(paste0("avMethMatHMLNormalized", ff), avMethMatHMLNormalized)
}

# for some reason, there are 4 bins that do not have any data in the 4th timepoint:
# TODO: check this
rowSums(is.na(avMethMatHMLNormalized4))
